...
int discover_throttle_values(physical_node_t* phys_node, bw_throttle_t** bw_throttlep)
{
         ...
         i++, throttle_reg_val += throttle_increment) 
    {
        ...
        phys_node->cpu_model->set_throttle_register(regs, THROTTLE_DDR_WRITE, throttle_reg_val);
...
int __set_read_bw(physical_node_t* node, uint64_t target_bw)
{
    ...
    node->cpu_model->set_throttle_register(regs, THROTTLE_DDR_WRITE, node->bw_throttle->throttle_reg_val[point]);
